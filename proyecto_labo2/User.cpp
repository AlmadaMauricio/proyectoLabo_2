...
void modificar_user()
{
	...
	while (!salir) {
		...
		do
		{
			cout << "Ingrese ID de user a modificar (0 para volver al menu anterior): ";
			cin >> idaux;
		...
		cout << "--------------------------" << endl;
		cout << "0 - Volver al menú Users" << endl << endl;
...
void baja_user()
{
	...
	do {
		...
		cout << "¿Desea dar de baja el user  N°" << id << "? (S/N): ";
		cin >> confirm;
...
void ordenarVectorPorFecha(User* vec, int tam) {
	...
	for (int i = 0; i < tam - 1; i++) {
		for (int j = i + 1; j < tam; j++) {
			if (vec[i].getFechaIngreso() > vec[j].getFechaIngreso()) {
				aux = vec[i];
				vec[i] = vec[j];
				vec[j] = aux;
			}
