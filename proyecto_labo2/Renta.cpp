...
bool Renta::grabarEnDisco() {
	...
	bool escribio = fwrite(this, sizeof(Renta), 1, fReg);
	fclose(fReg);
...
bool Renta::leerDeDisco(int pos) {
	...
	fseek(fReg, pos * sizeof(Renta), SEEK_SET);
	bool leyo = fread(this, sizeof(Renta), 1, fReg);
	fclose(fReg);
...
bool Renta::modificarEnDisco(int pos) {
	...
	fseek(fReg, pos * sizeof(Renta), SEEK_SET);
	bool leyo = fwrite(this, sizeof(Renta), 1, fReg);
	fclose(fReg);
	return leyo;
}
...
void RentasPendientesPorUser(int idUser) {
	...
	while (renta.leerDeDisco(p++))
	{
		if (renta.getIdUser() == idUser && renta.getAprobado() == 0)
		{
			renta.listarRenta();
		}
	}
...
void listarRentasPorIdDesc() {
	...
	ordenarVecRentaPorIdDesc(rentas, cantRentas);
...
void listarRentaPorFechaDesc() {
	...
	ordenarVectorRentaPorFechaDesc(vRentas, cantReg);
...
void bajaRenta(Renta aux, int pos)
{
	...
	if (confirm == 's')
	{
		/// Elimina el monopatin en archivo monopatines
...
void modificar_renta()
{
	...
	while (!salir) {
		...
		do
		{
			cout << "Ingrese ID de renta a modificar (0 para volver al menu Renta): ";
			cin >> idaux;
...
int rentasDesaprobadasPorAnio(int anio)
{
	...
	while (aux.leerDeDisco(p++))
	{
		if (aux.getEstado())
		{
			if (aux.getAprobado() == -1 && aux.getFechaRenta().getAnio() == anio)
			{
				cant++;
			}
		}
